...
def compute_coeff_classical_2(phi_classical_orbit, t_classical_orbit, Compression_Index,
                              Sigma_classical_average, tt, t1, N):
    """Compute Coefficient_classical_lambda_Fourier2"""
    
    # Construct the complex exponential signal using a positive imaginary exponent
    f = np.exp(1j * (1 / Compression_Index) *
               (phi_classical_orbit - t_classical_orbit / Sigma_classical_average))
    ...
    for iiiiii in range(1):
        if np.isclose(t[-1], 2 * np.pi, atol=1e-8):
            ...
            if np.isclose(t[0], 0, atol=1e-8):
                t_use = t[:-1]  # Remove the duplicate 2π point to avoid aliasing in FFT
                f_use = f[:-1]  # Corresponding signal values
                N = len(t_use)
            else:
                # Keep the full array as-is (not recommended for periodic Fourier analysis)
                t_use = t
                f_use = f
                N = len(t_use)
        else:
        ...
        # Map indices to a symmetric range around zero:
        ...
        n_shifted = np.fft.fftshift(n_unshifted.astype(int))  # Enforce integer data type
        ...
        if np.allclose(n_shifted, np.rint(n_shifted), atol=1e-10):
            ...
            # print("✅ Frequencies confirmed to be integers!")
        else:
            # print("❌ Frequencies are not integers; please check input data.")
            n_final = n_shifted
            
    # Return the reversed shifted Fourier coefficients
    # (common convention in some physics or gravitational-wave codes)
